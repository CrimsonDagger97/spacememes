package com.spacememes.enemy_types;
...
import com.spacememes.enemy_bullets.EnemyBullets;
import com.spacememes.game_screen.BasicBlocks;
import com.spacememes.game_screen.GameScreen;
...
import com.spacememes.sprite.Animation;
...
public class EnemyTypeBasic extends EnemyType{
	...
	public EnemyTypeBasic(double xPos, double yPos, int rows, int columns, EnemyBulletHandler bulletHandler){
		...
		enemySprite = new Animation(xPos, yPos, rows, columns, 300, "/com/spacememes/images/Invaders.png");
		...
		explosionSound = new Sound("/com/spacememes/sounds/explosion.wav");
	}
	...
	public void update(double delta, Player player, BasicBlocks blocks) {
		...
		if (shootTimer.timerEvent(shootTime)) {
			getBulletHandler().addBullet(new EnemyBullets(getRect().x, getRect().y));
			shootTime = new Random().nextInt(12000);
		}
	}
